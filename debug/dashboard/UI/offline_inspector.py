...
def render_offline_inspector(database_path: str) -> None:
    ...
    try:
        states, experiments, model_input_records = _load_memory(database_path)
    ...
    selected_experiments = matching_experiments(experiments, selected_state)
    selected_model_inputs = matching_model_input_records(
        model_input_records,
        selected_state,
    )
    render_selected_turn(selected_state, selected_experiments, selected_model_inputs)
...
def _load_memory(
    ...
    return (
        ...
        load_e_experiments(database_path),
...
def _filter_states(states: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    game_id = st.sidebar.selectbox("Game", game_ids)
