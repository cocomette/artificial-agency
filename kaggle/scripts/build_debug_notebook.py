...
VLLM_VERSION = os.environ.get("FACE_OF_AGI_KAGGLE_DEBUG_VLLM_VERSION", "0.19.1")
FULL_VLLM_DEPS = os.environ.get(
    "FACE_OF_AGI_KAGGLE_DEBUG_FULL_VLLM_DEPS",
    "",
...
VLLM_STACK_PACKAGES = (f"vllm=={VLLM_VERSION}",) if FULL_VLLM_DEPS else (
...
VLLM_SKIP_DEPENDENCY_NAMES = {"vllm"} if FULL_VLLM_DEPS else {
    "torch",
    "torchaudio",
    "torchvision",
    "triton",
...
if FULL_VLLM_DEPS:
    ...
    # available in Kaggle's offline wheel inputs. Install the pinned Torch
    # stack from the MiniCPM wheelhouse, then let the other vLLM CUDA packages
    # satisfy the runtime stack.
    VLLM_NO_DEPS_DEPENDENCY_NAMES = ("torch", "torchaudio", "torchvision")
else:
    # This package declares broad Torch/CUDA dependencies; keep Kaggle's
    # preinstalled stack authoritative and install only the matching wheel.
    VLLM_NO_DEPS_DEPENDENCY_NAMES = ("humming-kernels",)
...
def build(
    ...
    *,
    ...
    wheelhouse_dataset_slug: str = WHEELHOUSE_DATASET_SLUG,
) -> dict:
    ...
    return {
        ...
        "cells": [
            ...
            _install_cell(wheelhouse_dataset_slug=wheelhouse_dataset_slug),
...
def _install_cell(*, wheelhouse_dataset_slug: str = WHEELHOUSE_DATASET_SLUG) -> dict:
    return code_cell(
        dedent(
            f"""\
            import subprocess
            ...
            def pip_install_wheels(wheels, *, chunk_size=40):
                wheels = [str(wheel) for wheel in wheels]
                for start in range(0, len(wheels), chunk_size):
                    chunk = wheels[start:start + chunk_size]
                    if not chunk:
                        continue
            ...
            def selected_wheelhouse_wheels():
                selected = {{}}
                for path in wheelhouse_input.glob("*.whl"):
                    name, version, build, _tags = parse_wheel_filename(path.name)
                    normalized = canonicalize_name(name)
                    candidate = (version, str(build), path)
                    if normalized not in selected or candidate[:2] > selected[normalized][:2]:
                        selected[normalized] = candidate
                return [
                    candidate[2]
                    for _name, candidate in sorted(selected.items())
                ]
            ...
            def vllm_dependency_requirements():
                skipped = {VLLM_SKIP_DEPENDENCY_NAMES!r}
                no_deps_names = {set(VLLM_NO_DEPS_DEPENDENCY_NAMES)!r}
                ...
                for requirement in requirements:
                    ...
                    if canonicalize_name(parsed.name) in no_deps_names:
                        no_deps_requirements.append(requirement)
                    else:
                        deps_requirements.append(requirement)
                return deps_requirements, no_deps_requirements

            pip_install({RUNTIME_PACKAGES!r})
            if {FULL_VLLM_DEPS!r}:
                pip_install_wheels(selected_wheelhouse_wheels())
            else:
                pip_install({VLLM_TORCH_DEPENDENCY_PACKAGES!r}, pre=True)
                vllm_deps, vllm_no_deps = vllm_dependency_requirements()
                pip_install(vllm_no_deps, no_deps=True)
                pip_install(vllm_deps, pre=True)
...
def _sync_metadata(
    *,
    ...
    meta["dataset_sources"] = kaggle_dataset_sources(
        _dataset_source_slugs(
            model_dataset_slug,
            wheelhouse_dataset_slug=wheelhouse_dataset_slug,
        )
    )
    meta["model_sources"] = []
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--wheelhouse-dataset-slug", default=WHEELHOUSE_DATASET_SLUG)
...
def main(argv: list[str] | None = None) -> None:
    ...
    if args.metadata_only:
        meta = _sync_metadata(
            ...
            wheelhouse_dataset_slug=args.wheelhouse_dataset_slug,
