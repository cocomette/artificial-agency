...
REQUIRED_WHEELHOUSE_PACKAGES = ("pycountry", "pydantic-extra-types", "trl")
VLLM_STACK_PACKAGES = (
...
def _install_cell(*, wheelhouse_dataset_slug: str) -> dict:
    return code_cell(
        dedent(
            f"""\
            import subprocess
            ...
            def require_wheelhouse_wheels(distributions):
                ...
                if missing:
                    raise FileNotFoundError(
                        {wheelhouse_dataset_slug!r}
                        + " is missing required wheels: "
                        + ", ".join(missing)
                        + ". Rebuild and upload it with "
                        + "`cd kaggle && make debug-wheelhouse-upload "
                        + "UPLOAD_MODE=version`."
                    )
...
def _current_debug_wheelhouse_dataset_slug() -> str:
    return _debug_wheelhouse_dataset_slug(_current_branch_name())


def _debug_wheelhouse_dataset_slug(branch_name: str) -> str:
    return f"{WHEELHOUSE_DATASET_SLUG}-{_metadata_slug(branch_name)}"
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--wheelhouse-dataset-slug")
...
def main(argv: list[str] | None = None) -> None:
    ...
    wheelhouse_dataset_slug = (
        args.wheelhouse_dataset_slug.strip()
        if args.wheelhouse_dataset_slug and args.wheelhouse_dataset_slug.strip()
        else _current_debug_wheelhouse_dataset_slug()
    )
