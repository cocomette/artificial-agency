"""Manual E2E check for Ollama world prediction description predictions.
...
DEFAULT_OUTPUT_DIR = ROOT / "runs" / "ollama_world_description_e2e"
...
def main() -> None:
    ...
    action = ActionSpec(action_id=args.action)
    result = adapter.predict(
        context=RoleContext(game=args.context),
        action=action,
        observation=Observation(id="manual-world-source", step=0, frame=image),
    )
    ...
    print(f"saved Ollama world-description E2E result to {output_path}")
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--action", default="ACTION1")
    parser.add_argument("--context", default="Unknown transition rules.")
