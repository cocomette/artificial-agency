"""Manual E2E check for Ollama local image-change understanding.
...
DEFAULT_OUTPUT_DIR = ROOT / "runs" / "ollama_image_change_e2e"
    ...
    "items": {
        ...
        "properties": {
            "image_1_area_id": {
                "type": ["integer", "null"],
                "description": (
                    "ID of the related Image 1 description item, or null when "
                    "the change is only related to Image 2."
                ),
            },
            "image_2_area_id": {
                "type": ["integer", "null"],
                "description": (
                    "ID of the related Image 2 description item, or null when "
                    "the change is only related to Image 1."
                ),
            },
            "description": {
                "type": "string",
        ...
        "required": ["image_1_area_id", "image_2_area_id", "description"],
...
def main() -> None:
    ...
    target_description = describe_image_with_ollama(
        client=description_client,
        model=args.model,
        image=target_input,
    ...
    prompt = _prompt_text(
        args.source,
        args.target,
        image_size=source_input.size,
        image_1_description=_description_json_for_prompt(source_description),
        image_2_description=_description_json_for_prompt(target_description),
    ...
    changes, validation_errors = _validated_changes(
        ...
        image_2_ids={area["id"] for area in target_description.described_areas},
    )
    ...
    artifacts = _write_description_artifacts(
    ...
    summary = {
        ...
        "artifacts": artifacts,
        "image_descriptions": {
            "image_1": _description_summary(source_description),
            "image_2": _description_summary(target_description),
        },
        "prompt": prompt,
        ...
        "raw_response_object": jsonable(response),
        "response_shape": response_shape,
        "validation_errors": validation_errors,
        "usage": {
            "image_1_description": response_usage(source_description.response_object),
            "image_2_description": response_usage(target_description.response_object),
            "difference_description": response_usage(response),
        },
    }
    output_path = output_dir / "summary.json"
    output_path.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n")
    print(f"saved Ollama image-change E2E result to {output_path}")
    if changes:
...
def _prompt_text(
    ...
    *,
    ...
    image_1_description: Any,
    image_2_description: Any,
    schema: dict[str, Any],
) -> str:
    ...
    return (
        "You are comparing two frame descriptions.\n"
        f"Image 1 is the before frame.\n"
        f"Image 2 is the after frame.\n"
        "Each description item has a stable id, pixel bbox_2d array [x0, y0, x1, y1], and text description.\n"
        "You shall identify areas between before and after frame that changed.\n"
        "Return one array element per change. For each item, identify the relevant item id in each image and describe what changed.\n"
        "Use null for an image id if elements clearly appeared or disappeared.\n"
        "We are looking at relevant change such as but not limited to: orientation, movement, color, shape, appearance, disappearance.\n"
        "Image 1 description JSON:\n"
        f"{json.dumps(image_1_description, indent=2, sort_keys=True)}\n"
        "Image 2 description JSON:\n"
        f"{json.dumps(image_2_description, indent=2, sort_keys=True)}\n"
...
def _description_json_for_prompt(result: ImageDescriptionCallResult) -> Any:
    return result.described_areas


def _description_summary(result: ImageDescriptionCallResult) -> dict[str, Any]:
    return {
        "prompt": result.prompt,
        "assistant_prefill": ASSISTANT_JSON_PREFILL,
        "raw_response": result.raw_text,
        "described_areas": result.described_areas,
        "raw_response_object": jsonable(result.response_object),
        "validation_errors": result.validation_errors,
        "usage": response_usage(result.response_object),
    }
...
def _validated_changes(
    ...
    *,
    ...
    for index, item in enumerate(parsed_response, start=1):
        ...
        image_2_area_id, image_2_error = _validated_area_id(
            item.get("image_2_area_id"),
            label=f"item {index} image_2_area_id",
            valid_ids=image_2_ids,
        )
        ...
        if image_1_error is not None or image_2_error is not None:
            continue
        if image_1_area_id is None and image_2_area_id is None:
            errors.append(f"item {index}: at least one image area id must be non-null")
            continue
        changes.append(
            {
                "image_1_area_id": image_1_area_id,
                "image_2_area_id": image_2_area_id,
...
def _write_description_artifacts(
    *,
    output_dir: Path,
    source: Image.Image,
    target: Image.Image,
    source_description: ImageDescriptionCallResult,
    target_description: ImageDescriptionCallResult,
) -> dict[str, str]:
    ...
    annotated_target_path = output_dir / "image_2_annotated.png"
    annotated_area_image(source, source_description.described_areas).save(
        annotated_source_path
    )
    annotated_area_image(target, target_description.described_areas).save(
        annotated_target_path
    )
    return {
        ...
        "image_2_annotated": str(annotated_target_path.relative_to(output_dir)),
    }
