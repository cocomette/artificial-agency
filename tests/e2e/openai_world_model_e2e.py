"""Manual E2E check for OpenAI world prediction description predictions.

This script calls the real OpenAI API. Set OPENAI_API_KEY before running it.
It uses a committed ARC source frame, asks S for a next-state description, and
...
def main() -> None:
    ...
    context = RoleContext(
        general=(
            ...
            "action as structured area descriptions."
        ),
        game=_compose_game_context(),
    )
    adapter = WorldPredictionAdapter(_config_from_args(args))
    ...
    _validate_action_in_prompt(adapter.last_prompt or "", action)
    ...
    print(f"saved OpenAI world-description E2E result to {output_path}")
...
def _compose_game_context() -> str:
    """Return fixture-specific world context."""
...
def _validate_action_in_prompt(prompt: str, action: ActionSpec) -> None:
    ...
    if action_id is None or _normalized(action_id) not in _normalized(prompt):
