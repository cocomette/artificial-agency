...
def test_vllm_agent_selection_uses_static_model_and_records_metadata() -> None:
    current = _observation("obs-current")
    client = FakeOpenAIChatClient(json.dumps({"action": {"action_id": "ACTION1"}}))
    adapter = VLLMOrchestratorAgentAdapter(
        VLLMOrchestratorAgentConfig(
    ...
    assert decision.final_action.action_id == "ACTION1"
    assert decision.trace.metadata["agent_v1"] is True
    assert decision.trace.metadata["candidate_count"] == 1
    assert decision.trace.metadata["world_prediction_count"] == 1
    assert decision.trace.metadata["interest_value_count"] == 1
    assert decision.trace.metadata["usage"]["total_tokens"] == 6
...
def test_vllm_world_uses_static_model_and_records_backend_metadata() -> None:
    current = _observation("obs-current")
    client = FakeOpenAIChatClient(
    ...
    assert prediction.metadata["backend"] == "vllm"
    assert request["model"] == MODEL
...
def test_vllm_json_roles_include_schema_in_instructions_and_not_extra_body() -> None:
    ...
    assert interest_prediction.metadata["backend"] == "vllm"
