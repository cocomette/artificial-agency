...
def test_world_prediction_returns_description_result() -> None:
    ...
    assert "You are the world model for an ARC-AGI-3 game agent." in system_prompt
    assert "ROLE_CONTEXT:" in prompt
    assert "General world facts." in prompt
    assert "Game dynamics." in prompt
    assert "Action glossary:" in system_prompt
    assert (
        "`ACTION6`: coordinate action targeting `x,y` on the 64x64 game grid."
        in system_prompt
    )
    assert "`ACTION7`: undo-style simple action." in system_prompt
...
def test_world_prediction_empty_context_keeps_context_section_blank() -> None:
    client = FakeOllamaClient()
    adapter = WorldPredictionAdapter(
        config=OllamaDescriptionConfig(model="gemma4:e4b", input_image_size="64x64"),
        client=client,
    ...
    assert user_prompt.startswith("ROLE_CONTEXT:\n\nACTION:")
