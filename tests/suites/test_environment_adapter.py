...
class _StepRecordingEnvironment:
    def __init__(self, raw_observation=None) -> None:
    ...
    def step(self, action_id, *, data=None, reasoning=None):
        self.steps.append((action_id, data, reasoning))
        return self.raw_observation
...
def test_precreated_environment_wrapper_submits_string_action_as_arc_enum() -> None:
    environment = _StepRecordingEnvironment(_raw_observation())
    adapter = ArcEnvironmentWrapperAdapter(
        game_id="game-1",
        environment=environment,
    )

    adapter.step(ActionSpec(action_id="ACTION1"))

    assert environment.steps[0][0] is GameAction.ACTION1


def test_precreated_environment_wrapper_step_failure_reports_action_name() -> None:
    environment = _StepRecordingEnvironment(raw_observation=None)
    adapter = ArcEnvironmentWrapperAdapter(
        game_id="game-1",
        environment=environment,
    )

    with pytest.raises(RuntimeError, match="with action 'ACTION1'"):
        adapter.step(ActionSpec(action_id="ACTION1"))
