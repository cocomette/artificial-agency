...
from face_of_agi.runtime.loop import _persist_model_input_debug_records
...
def test_runtime_model_input_debug_persistence_follows_operation_mode() -> None:
    competition = EnvironmentConfig(
        game_index=0,
        max_actions_per_level=1,
        operation_mode=OperationMode.COMPETITION,
    )
    offline = EnvironmentConfig(
        game_index=0,
        max_actions_per_level=1,
        operation_mode=OperationMode.OFFLINE,
    )

    assert _persist_model_input_debug_records(competition) is False
    assert _persist_model_input_debug_records(offline) is True
...
def test_kaggle_runner_retries_failed_game_with_same_environment(
    monkeypatch,
    tmp_path,
) -> None:
    config_path = tmp_path / "kaggle.yaml"
    ...
    def fake_run_game(spec: ParallelGameRunSpec, trace_output):
        ...
        if spec.game_id == "vc33-bbb" and attempts[spec.game_id] == 1:
    ...
    assert retry_success.run_id == "kaggle-test-vc33-bbb-retry-1"
    assert retry_success.database_path.endswith("memory-vc33-bbb-retry-1.sqlite")
    assert [spec.run_id for spec in captured_specs if spec.game_id == "vc33-bbb"] == [
        "kaggle-test-vc33-bbb",
        "kaggle-test-vc33-bbb-retry-1",
    ]
    assert [spec.attempt_index for spec in captured_specs if spec.game_id == "vc33-bbb"] == [
        0,
        1,
    ]
    first_spec, retry_spec = [
        spec for spec in captured_specs if spec.game_id == "vc33-bbb"
    ]
    assert retry_spec.arc_environment is first_spec.arc_environment
    assert fake_arcade.made == [
        ("ls20-aaa", "scorecard-1"),
        ("vc33-bbb", "scorecard-1"),
    ]
...
def test_kaggle_runner_closes_scorecard_after_retry_failure(
    monkeypatch,
    tmp_path,
) -> None:
    config_path = tmp_path / "kaggle.yaml"
    ...
    assert failure.run_id == "kaggle-test-vc33-bbb-retry-1"
    assert failure.database_path.endswith("memory-vc33-bbb-retry-1.sqlite")
    assert fake_arcade.made == [
        ("ls20-aaa", "scorecard-1"),
        ("vc33-bbb", "scorecard-1"),
    ]
    ...
    result = ParallelGameRunResult(
        ...
        failures=(
            ParallelGameRunFailure(
                game_index=1,
                game_id="vc33-bbb",
                run_id="run-1-retry-1",
                database_path=str(tmp_path / "memory-vc33-bbb-retry-1.sqlite"),
