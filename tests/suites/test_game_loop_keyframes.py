...
_WORLD_TRAINING_REQUEST = {
    "model": "base-model",
    "messages": [
        {"role": "system", "content": "world instructions"},
        {
            "role": "user",
            "content": [
                {"type": "text", "text": "world prompt"},
    ...
    "response_format": {
        "type": "json_schema",
        "json_schema": {"name": "world_prediction", "schema": {}},
    },
}

_AGENT_TRAINING_REQUEST = {
    "model": "base-model",
    "messages": [
    ...
    "response_format": {
        "type": "json_schema",
        "json_schema": {"name": "agent_final_action", "schema": {}},
    },
}

_INTEREST_TRAINING_REQUEST = {
    "model": "base-model",
    "messages": [
        {"role": "system", "content": "interest instructions"},
        {
            "role": "user",
            "content": [
                {"type": "text", "text": "interest prompt"},
                {"type": "image_url", "image_url": {"url": "data:image/png;base64,AA=="}},
            ],
        },
    ],
    "response_format": {
        "type": "json_schema",
        "json_schema": {"name": "interest_prediction", "schema": {}},
    },
}
...
def test_real_turn_persists_reward_and_sanitizes_memory_ledger(
    ...
    assert reward["learning_progress"] is None
    assert reward["goal_delta"] > 0.0
    assert reward["total"] > reward["prediction_accuracy"] * reward["lp_weight"]

    world_replay = state_memory.list_replay_samples(
        run_id="run-1",
        game_id="game-1",
        role="world",
    )[0]
    agent_replay = state_memory.list_replay_samples(
        run_id="run-1",
        game_id="game-1",
        role="agent",
    )[0]
    interest_replay = state_memory.list_replay_samples(
        run_id="run-1",
        game_id="game-1",
        role="interest",
    )[0]
    assert world_replay.prompt_json["request"] == _WORLD_TRAINING_REQUEST
    assert world_replay.completion_json["target"] == {"predicted_change": "changed"}
    assert world_replay.held_out is False
    assert world_replay.metadata["learning_progress_eval"]["change_summary"] == (
        "changed"
    )
    assert world_replay.metadata["learning_progress_eval"]["candidate_index"] == 0
    assert "previous_observation" in world_replay.metadata["learning_progress_eval"]
    assert "current_observation" in world_replay.metadata["learning_progress_eval"]
    assert agent_replay.prompt_json["request"] == _AGENT_TRAINING_REQUEST
    assert interest_replay.prompt_json["request"] == _INTEREST_TRAINING_REQUEST
    assert agent_replay.completion_json["target"] == {
        "action": {"action_id": "ACTION1", "data": None}
    }
    assert agent_replay.held_out is False
    assert agent_replay.metadata["reward_components"]["lp_weight"] == reward[
        "lp_weight"
    ]
    assert agent_replay.metadata["reward_components"]["goal_delta"] == reward[
        "goal_delta"
    ]
    assert interest_replay.metadata["executed_candidate_index"] == 0
    assert interest_replay.metadata["candidate_score_table"][0]["blended_score"] > 0
    ...
    def select_action(
        ...
        *,
        ...
        trace = AgentTrace(
            ...
            final_action=action,
            metadata={"training_request": _AGENT_TRAINING_REQUEST},
        )
...
class _FakeWorld:
    def predict_transition(self, prediction_input) -> WorldPrediction:
        return WorldPrediction(
            ...
            metadata={"training_request": _WORLD_TRAINING_REQUEST},
        )
...
class _FakeInterest:
    def score_candidates(self, prediction_input) -> InterestPrediction:
        ...
        return InterestPrediction(
            ...
            metadata={"training_request": _INTEREST_TRAINING_REQUEST},
        )
