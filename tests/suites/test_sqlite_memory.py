...
def test_v1_runtime_tables_round_trip(tmp_path) -> None:
    ...
    assert reward_record.reward["learning_progress"] == 0.9
    assert ledger.change_summary == "object moved"
    with sqlite3.connect(memory.database.path) as connection:
        tables = {
            row[0]
    ...
    assert "replay_samples" not in tables
    assert "lora_updates" not in tables
