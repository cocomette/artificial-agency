...
def test_kaggle_notebook_uses_rtx6000_and_offline_inputs() -> None:
    ...
    assert (
        "require_wheelhouse_wheels(('pycountry', 'pydantic-extra-types'))"
        in sources
    )
    assert "wheelhouse-upload UPLOAD_MODE=version" in sources
    assert "pip_install(('arc-agi', 'arcengine'" in sources
...
def test_kaggle_wheelhouse_builder_prunes_kaggle_torch_stack(
    ...
    wheelhouse.mkdir()
    keep = wheelhouse / "pandas-2.3.3-py3-none-any.whl"
...
def test_kaggle_rtx6000_debug_config_loads_public_debug_runtime() -> None:
    ...
    assert _extra_arg_value(vllm_config.extra_args, "--max-num-seqs") == "64"
    assert (
        _extra_arg_value(vllm_config.extra_args, "--max-num-batched-tokens")
        == "65536"
    )
    assert _extra_arg_value(vllm_config.extra_args, "--gpu-memory-utilization") == "0.90"
    ...
    assert config.models.shared_vlm.model == "Qwen/Qwen3.6-35B-A3B-FP8"
