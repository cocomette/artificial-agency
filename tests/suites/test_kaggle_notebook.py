...
KAGGLE_KERNEL_TITLE_MAX_LENGTH = 48


@pytest.fixture(autouse=True)
def _kaggle_test_env(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    token_path = tmp_path / "kaggle-token"
    token_path.write_text("fake-token", encoding="utf-8")
    monkeypatch.setenv("FACE_OF_AGI_KAGGLE_OWNER", "kaggle-owner")
    monkeypatch.setenv("FACE_OF_AGI_KAGGLE_TOKEN_FILE", str(token_path))
...
def test_kaggle_env_uses_environment_without_local_env() -> None:
    helper = _load_script("kaggle/scripts/kaggle_env.py")

    assert helper.kaggle_owner() == "kaggle-owner"
    assert helper.kaggle_token_path().name == "kaggle-token"
...
def test_kaggle_notebook_uses_rtx6000_and_offline_inputs() -> None:
    ...
    assert "kaggle_dataset_input('face-of-agi-wheelhouse')" in sources
    ...
    assert "kaggle_torch_stack_constraints.txt" in sources
    assert '"--constraint"' in sources
    assert '"--pre"' in sources
    assert 'write_installed_constraints(("torch", "torchaudio", "torchvision", "triton"))' in sources
    assert "'datasets'" in sources
    assert "'arcengine'" in sources
    assert "'peft'" in sources
    assert "'pillow'" in sources
    assert "'pycountry'" in sources
    assert "'pydantic-extra-types'" in sources
    assert "'trl'" in sources
    assert (
        "require_wheelhouse_wheels(('pycountry', 'pydantic-extra-types', 'trl'))"
        in sources
    )
    assert "wheelhouse-upload UPLOAD_MODE=version" in sources
    assert "pip_install(('accelerate', 'arc-agi', 'arcengine'" in sources
    assert "allow_pre=True" in sources
    assert "vllm==0.19.1" in sources
    assert "xgrammar==0.2.1" in sources
    assert "vllm_server_command" in sources
    assert "VLLM_ALLOW_RUNTIME_LORA_UPDATING" in sources
...
def test_kaggle_notebook_can_target_gemma_submission(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    monkeypatch.setenv("FACE_OF_AGI_KAGGLE_OWNER", "kaggle-owner")
    monkeypatch.setenv(
        "FACE_OF_AGI_KAGGLE_SUBMISSION_CONFIG",
        "src/face_of_agi/runtime/configs/vllm/vllm_rtx6000_gemma4_parallel.yaml",
    )
    monkeypatch.setenv(
        "FACE_OF_AGI_KAGGLE_MODEL_DATASET_SLUG",
        "face-of-agi-gemma4-31b-it-qat-w4a16-ct-weights",
    )
    monkeypatch.setenv(
        "FACE_OF_AGI_KAGGLE_SUBMISSION_KERNEL_ID",
        "kaggle-owner/face-of-agi-arc-agi-3-rtx6000-gemma4",
    )
    monkeypatch.setenv(
        "FACE_OF_AGI_KAGGLE_SUBMISSION_KERNEL_TITLE",
        "FACE-OF-AGI ARC-AGI-3 RTX6000 Gemma4",
    )
    builder = _load_script("kaggle/scripts/build_notebook.py")
    ...
    assert builder.CONFIG_RELATIVE_PATH.endswith("vllm_rtx6000_gemma4_parallel.yaml")
    assert builder.MODEL_DATASET_SLUG == (
        "face-of-agi-gemma4-31b-it-qat-w4a16-ct-weights"
    )
    assert "vllm_rtx6000_gemma4_parallel.yaml" in sources
    assert (
        "kaggle_dataset_input('face-of-agi-gemma4-31b-it-qat-w4a16-ct-weights')"
        in sources
    )
    assert builder.DATASET_SOURCE_SLUGS == (
        "face-of-agi-wheelhouse",
        "face-of-agi-gemma4-31b-it-qat-w4a16-ct-weights",
    )
    assert synced is not None
    assert synced["id"] == "kaggle-owner/face-of-agi-arc-agi-3-rtx6000-gemma4"
    assert synced["title"] == "FACE-OF-AGI ARC-AGI-3 RTX6000 Gemma4"
    assert synced["dataset_sources"] == [
        "kaggle-owner/face-of-agi-wheelhouse",
        "kaggle-owner/face-of-agi-gemma4-31b-it-qat-w4a16-ct-weights",
    ]
...
def test_kaggle_debug_notebook_uses_shell_and_public_game_input() -> None:
    ...
    wheelhouse_slug = "face-of-agi-wheelhouse-feat-world-goal-agent"

    notebook = builder.build(
        source_archive_b64="dGVzdA==",
        wheelhouse_dataset_slug=wheelhouse_slug,
    )
    ...
    sources = "\n".join(cell["source"] for cell in notebook["cells"])
    assert f"kaggle_dataset_input('{wheelhouse_slug}')" in sources
    ...
    assert "debug-wheelhouse-upload " in sources
    assert "UPLOAD_MODE=version" in sources
    assert "pip_install(('accelerate', 'arc-agi', 'arcengine'" in sources
    assert "allow_pre=True" in sources
    assert "vllm==0.19.1" in sources
    assert "xgrammar==0.2.1" in sources
...
def test_modal_model_dataset_metadata_uses_dataset_slug_env(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv(
        "FACE_OF_AGI_KAGGLE_MODEL_DATASET_SLUG",
        "face-of-agi-qwen3-vl-4b-thinking-weights",
    )
    uploader = _load_script("kaggle/scripts/upload_modal_model_to_kaggle.py")
    metadata = json.loads(
        uploader._metadata_text(
            Path(
                "kaggle/upload/model-dataset/"
                "qwen3-vl-4b-thinking-dataset-metadata.json"
            ),
            artifact_kind="dataset",
        )
    )
    ...
    assert uploader._dataset_ref(metadata) == (
        f"{owner}/face-of-agi-qwen3-vl-4b-thinking-weights"
    )
...
def test_kaggle_debug_kernel_metadata_declares_offline_sources() -> None:
    metadata = json.loads(
        Path("kaggle/debug-notebooks/kernel-metadata.template.json").read_text()
    )
...
def test_kaggle_wheelhouse_builder_accepts_dataset_slug(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    builder = _load_script("kaggle/scripts/build_wheelhouse.py")
    metadata_path = tmp_path / "dataset-metadata.json"
    requirements_path = tmp_path / "requirements.txt"
    output_dir = tmp_path / "wheelhouse"
    metadata_path.write_text(
        '{"id": "old-owner/old-wheelhouse", "title": "Wheelhouse"}\n',
        encoding="utf-8",
    )
    requirements_path.write_text("trl\n", encoding="utf-8")
    calls = []
    monkeypatch.setattr(
        builder,
        "_pip_download",
    ...
    monkeypatch.setattr(builder, "_vllm_dependency_requirements", lambda output: [])

    builder.build_wheelhouse(
        SimpleNamespace(
            ...
            dataset_slug="face-of-agi-wheelhouse-feat-world-goal-agent",
            output=output_dir,
        )
    )

    assert json.loads((output_dir / "dataset-metadata.json").read_text()) == {
        "id": "kaggle-owner/face-of-agi-wheelhouse-feat-world-goal-agent",
        "title": "Wheelhouse",
    }
    assert calls[0][1].get("allow_pre") is None
    assert calls[1][1].get("allow_pre") is None
    assert calls[2][1]["allow_pre"] is True


def test_kaggle_wheelhouse_builder_prunes_kaggle_torch_stack(
    tmp_path: Path,
) -> None:
    builder = _load_script("kaggle/scripts/build_wheelhouse.py")
    wheelhouse = tmp_path / "wheelhouse"
    wheelhouse.mkdir()
    keep = wheelhouse / "trl-1.7.0-py3-none-any.whl"
    keep.write_text("", encoding="utf-8")
    for filename in (
        "cuda_bindings-13.3.1-cp312-cp312-manylinux_2_28_x86_64.whl",
        "torch-2.12.1-cp312-cp312-manylinux_2_28_x86_64.whl",
        "torchaudio-2.12.1-cp312-cp312-manylinux_2_28_x86_64.whl",
        "torchvision-0.27.1-cp312-cp312-manylinux_2_28_x86_64.whl",
        "triton-3.6.0-cp312-cp312-manylinux_2_28_x86_64.whl",
        "cuda_toolkit-13.0.2-py3-none-manylinux_2_28_x86_64.whl",
    ):
        (wheelhouse / filename).write_text("", encoding="utf-8")

    builder._remove_kaggle_torch_stack_wheels(wheelhouse)

    assert sorted(path.name for path in wheelhouse.glob("*.whl")) == [keep.name]
...
def test_kaggle_debug_kernel_identity_uses_branch_and_commit() -> None:
    ...
    assert kernel_id == "kaggle-owner/debug-feat-2-model-setup-5109ce7b"
    assert title == "Debug feat-2-model-setup 5109ce7b"
    assert len(title) <= KAGGLE_KERNEL_TITLE_MAX_LENGTH
    assert _kaggle_slug(title) == kernel_id.split("/", 1)[1]


def test_kaggle_debug_kernel_identity_shortens_long_branch_names() -> None:
    builder = _load_script("kaggle/scripts/build_debug_notebook.py")

    kernel_id, title = builder._debug_kernel_identity(
        "kaggle-owner",
        "feature/some-extremely-long-debug-notebook-identity",
        "5109ce7b",
    )

    assert title == "Debug feature-some-extremely-long-debug 5109ce7b"
    assert len(title) <= KAGGLE_KERNEL_TITLE_MAX_LENGTH
    assert _kaggle_slug(title) == kernel_id.split("/", 1)[1]


def test_kaggle_debug_kernel_identity_includes_variant() -> None:
    builder = _load_script("kaggle/scripts/build_debug_notebook.py")
    ...
    kernel_id, title = builder._debug_kernel_identity(
        ...
        "feat/2-model-setup",
        "5109ce7b",
        kernel_title_suffix="gemma4",
    )

    assert kernel_id == (
        "kaggle-owner/debug-gemma4-feat-2-model-setup-5109ce7b"
    )
    assert title == "Debug gemma4 feat-2-model-setup 5109ce7b"
    assert len(title) <= KAGGLE_KERNEL_TITLE_MAX_LENGTH
    assert _kaggle_slug(title) == kernel_id.split("/", 1)[1]


def test_kaggle_debug_wheelhouse_slug_uses_branch_name() -> None:
    builder = _load_script("kaggle/scripts/build_debug_notebook.py")

    assert builder._debug_wheelhouse_dataset_slug("feat/world-goal-agent") == (
        "face-of-agi-wheelhouse-feat-world-goal-agent"
    )


def test_kaggle_debug_builder_can_target_gemma_config(
) -> None:
    builder = _load_script("kaggle/scripts/build_debug_notebook.py")

    config_path = "src/face_of_agi/runtime/configs/vllm/vllm_rtx6000_gemma4_debug.yaml"
    model_slug = "face-of-agi-gemma4-31b-it-qat-w4a16-ct-weights"
    wheelhouse_slug = "face-of-agi-wheelhouse-feat-world-goal-agent"
    notebook = builder.build(
        source_archive_b64="dGVzdA==",
        config_relative_path=config_path,
        model_dataset_slug=model_slug,
        wheelhouse_dataset_slug=wheelhouse_slug,
    )
    sources = "\n".join(cell["source"] for cell in notebook["cells"])
    kernel_id, title = builder._debug_kernel_identity(
    ...
    assert "vllm_rtx6000_gemma4_debug.yaml" in sources
    assert (
        "kaggle_dataset_input('face-of-agi-gemma4-31b-it-qat-w4a16-ct-weights')"
        in sources
    )
    assert builder.kaggle_dataset_sources(
        builder._dataset_source_slugs(
            model_slug,
            wheelhouse_dataset_slug=wheelhouse_slug,
        )
    ) == (
        [
            "kaggle-owner/face-of-agi-wheelhouse-feat-world-goal-agent",
            "kaggle-owner/face-of-agi-public-games",
            "kaggle-owner/face-of-agi-gemma4-31b-it-qat-w4a16-ct-weights",
        ]
    )
    assert kernel_id == (
...
def test_kaggle_debug_metadata_sync_generates_kernel_identity(
    ...
    builder = _load_script("kaggle/scripts/build_debug_notebook.py")
    metadata_path = tmp_path / "kernel-metadata.json"
    template_path = tmp_path / "kernel-metadata.template.json"
    metadata = json.loads(
        Path("kaggle/debug-notebooks/kernel-metadata.template.json").read_text(
            encoding="utf-8"
        )
    ...
    monkeypatch.setattr(builder, "METADATA_PATH", metadata_path)
    monkeypatch.setattr(builder, "METADATA_TEMPLATE_PATH", template_path)
    monkeypatch.setattr(builder, "kaggle_owner", lambda: "kaggle-owner")
    monkeypatch.setattr(
        builder,
        "_current_debug_kernel_identity",
        lambda owner, *, kernel_title_suffix="": builder._debug_kernel_identity(
            owner,
            "feat/2-model-setup",
            "5109ce7b",
    ...
    synced = builder._sync_metadata(
        wheelhouse_dataset_slug="face-of-agi-wheelhouse-feat-2-model-setup",
        kernel_title_suffix="qwen36",
    )

    assert synced["id"] == "kaggle-owner/debug-qwen36-feat-2-model-setup-5109ce7b"
    assert synced["title"] == "Debug qwen36 feat-2-model-setup 5109ce7b"
    assert json.loads(metadata_path.read_text(encoding="utf-8")) == synced
    assert len(synced["title"]) <= KAGGLE_KERNEL_TITLE_MAX_LENGTH
    ...
    assert [source.split("/", 1)[1] for source in synced["dataset_sources"]] == [
        "face-of-agi-wheelhouse-feat-2-model-setup",
        "face-of-agi-public-games",
        "face-of-agi-qwen36-35b-fp8-weights",
    ]


def test_kaggle_debug_builder_metadata_only_cli(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    builder = _load_script("kaggle/scripts/build_debug_notebook.py")
    metadata_path = tmp_path / "kernel-metadata.json"
    notebook_path = tmp_path / "debug.ipynb"
    template_path = Path("kaggle/debug-notebooks/kernel-metadata.template.json")

    monkeypatch.setattr(builder, "METADATA_PATH", metadata_path)
    monkeypatch.setattr(builder, "NOTEBOOK_PATH", notebook_path)
    monkeypatch.setattr(builder, "METADATA_TEMPLATE_PATH", template_path)
    monkeypatch.setattr(builder, "kaggle_owner", lambda: "kaggle-owner")
    monkeypatch.setattr(
        builder,
    ...
    assert notebook_path.exists() is False
    assert [source.split("/", 1)[1] for source in metadata["dataset_sources"]] == [
        "face-of-agi-wheelhouse-feat-2-model-setup",
        "face-of-agi-public-games",
        "face-of-agi-gemma4-31b-it-qat-w4a16-ct-weights",
...
def test_kaggle_kernel_ref_resolver_accepts_url_slug_and_metadata(
    tmp_path: Path,
) -> None:
    resolver = _load_script("kaggle/scripts/resolve_kaggle_kernel_ref.py")
    metadata_path = tmp_path / "kernel-metadata.json"
    metadata_path.write_text('{"id": "owner/from-metadata"}\n', encoding="utf-8")

    assert resolver._kernel_id("owner/direct-slug") == "owner/direct-slug"
    assert (
        resolver._kernel_id("https://www.kaggle.com/code/owner/from-url?scriptVersionId=1")
        == "owner/from-url"
    )
    assert resolver._metadata_kernel_id(metadata_path) == "owner/from-metadata"
...
def test_kaggle_submit_targets_request_rtx_pro6000_accelerator() -> None:
    ...
    assert (
        "KAGGLE_SUBMISSION_CONFIG ?= src/face_of_agi/runtime/configs/vllm/"
        "vllm_rtx6000_qwen36_35b_fp8_parallel.yaml"
        in makefile
    )
    assert (
        "GEMMA_SUBMISSION_CONFIG ?= src/face_of_agi/runtime/configs/vllm/"
        "vllm_rtx6000_gemma4_parallel.yaml"
        in makefile
    )
    assert (
        "GEMMA_SUBMISSION_MODEL_DATASET_SLUG ?= "
        "face-of-agi-gemma4-31b-it-qat-w4a16-ct-weights"
        in makefile
    )
    assert "gemma-submit: gemma-notebook _check-kaggle" in makefile
    assert '$(KAGGLE) kernels status "$(GEMMA_SUBMISSION_KERNEL_ID)"' in makefile
    assert (
        "KAGGLE_DEBUG_CONFIG ?= src/face_of_agi/runtime/configs/vllm/"
    ...
    assert "KAGGLE_DEBUG_BRANCH_SLUG ?=" in makefile
    assert (
        "KAGGLE_DEBUG_WHEELHOUSE_DATASET_SLUG ?= "
        "face-of-agi-wheelhouse-$(KAGGLE_DEBUG_BRANCH_SLUG)"
        in makefile
    )
    assert "DEBUG_KERNEL_TITLE_SUFFIX ?=" in makefile
    assert "DEBUG_KERNEL ?=" in makefile
    assert "DEBUG_WHEELHOUSE_DIR := build/debug-wheelhouse" in makefile
    assert "debug-wheelhouse: sync-metadata" in makefile
    assert "debug-wheelhouse-upload: _check-kaggle debug-wheelhouse" in makefile
    assert '--dataset-slug "$(KAGGLE_DEBUG_WHEELHOUSE_DATASET_SLUG)"' in makefile
    assert (
        '--wheelhouse-dataset-slug "$(KAGGLE_DEBUG_WHEELHOUSE_DATASET_SLUG)"'
        in makefile
    )
    assert "debug-metadata: sync-metadata" in makefile
    assert "debug-submit: debug-notebook _check-kaggle" in makefile
    assert "scripts/resolve_kaggle_kernel_ref.py" in makefile
    assert (
...
def test_kaggle_rtx6000_debug_config_loads_public_debug_runtime() -> None:
    ...
    assert config.max_parallel_games is None
    ...
    assert _extra_arg_value(vllm_config.extra_args, "--max-num-seqs") == "16"
    assert (
        _extra_arg_value(vllm_config.extra_args, "--max-num-batched-tokens")
        == "8192"
    )
    assert _extra_arg_value(vllm_config.extra_args, "--gpu-memory-utilization") == "0.65"
    assert _extra_arg_value(vllm_config.extra_args, "--max-cpu-loras") == "512"
    assert _extra_arg_value(vllm_config.extra_args, "--chat-template-content-format") == "openai"
    assert "--swap-space" not in vllm_config.extra_args
    assert "--enable-prefix-caching" in vllm_config.extra_args
    assert vllm_config.max_model_len == 16384
    assert vllm_config.max_loras == 8
    assert vllm_config.max_lora_rank == 8
    assert vllm_config.allow_runtime_lora_updating is True
    ...
    assert config.models.world.options["include_output_schema_in_instructions"] is True
    assert config.models.world.options["max_completion_tokens"] == 3072
    assert (
        config.models.memory.options["include_output_schema_in_instructions"]
        is True
    )
    assert (
        config.models.goal.options["include_output_schema_in_instructions"]
        is True
    )
    assert (
        config.models.interest.options["include_output_schema_in_instructions"]
        is True
    )
    assert (
        config.models.reward_judge.options["include_output_schema_in_instructions"]
        is True
    )
    assert config.models.agent.options["max_completion_tokens"] == 2048
    assert config.models.change.options["max_completion_tokens"] == 2048
    assert config.models.memory.options["max_completion_tokens"] == 3072
    assert config.models.world.options["max_completion_tokens"] == 3072
    assert config.models.goal.options["max_completion_tokens"] == 2048
    assert config.models.interest.options["max_completion_tokens"] == 2048
    assert config.models.reward_judge.options["max_completion_tokens"] == 1024
    assert config.online_lora.base_model_path == (
        "/kaggle/input/face-of-agi-qwen36-35b-fp8-weights"
    )
    assert config.online_lora.trainer_base_model_path == (
        "/kaggle/input/face-of-agi-qwen36-35b-trainable-weights"
    )
    ...
    assert config.online_lora.max_concurrent_trainer_jobs == 1
    assert config.online_lora.min_new_samples_per_update == 8
    assert config.online_lora.max_update_wait_seconds == 30.0
    assert config.online_lora.max_train_samples == 8
    assert config.online_lora.trainer_cache_enabled is False
    assert vllm_config.enable_lora is True
...
def test_kaggle_rtx6000_submission_config_sets_reasoning_config() -> None:
    ...
    assert vllm_config is not None
    assert _extra_arg_value(vllm_config.extra_args, "--reasoning-config") == "{}"
    assert _extra_arg_value(vllm_config.extra_args, "--max-num-seqs") == "128"
    assert (
        _extra_arg_value(vllm_config.extra_args, "--max-num-batched-tokens")
        == "65536"
    )
    assert vllm_config.max_model_len == 32768
    assert config.models.agent.options["input_image_crop_arc_grid_edges"] == 4
    assert config.models.change.options["input_image_crop_arc_grid_edges"] == 4
    assert config.models.world.options["input_image_crop_arc_grid_edges"] == 4
    assert config.models.interest.options["input_image_crop_arc_grid_edges"] == 4
    assert config.models.reward_judge.options["input_image_crop_arc_grid_edges"] == 4
    assert config.models.agent.options["max_completion_tokens"] == 2048
    assert config.models.change.options["max_completion_tokens"] == 2048
    assert config.models.memory.options["max_completion_tokens"] == 3072
    assert config.models.world.options["max_completion_tokens"] == 2048
    assert config.models.goal.options["max_completion_tokens"] == 2048
    assert config.models.interest.options["max_completion_tokens"] == 1024
    assert config.models.reward_judge.options["max_completion_tokens"] == 1024
    assert config.online_lora.base_model_path == (
        "/kaggle/input/face-of-agi-qwen36-35b-fp8-weights"
    )
