...
def test_debug_bus_can_drain_model_input_records_without_persisting(tmp_path) -> None:
    ...
    assert bus.model_token_usage(run_id="run-1", game_id="game-1", turn_id=1) == {
        "prompt_tokens": 1,
        "completion_tokens": 2,
        "total_tokens": 3,
    }
    bus.clear_model_token_usage(run_id="run-1", game_id="game-1", turn_id=1)
    assert bus.model_token_usage(run_id="run-1", game_id="game-1", turn_id=1) == {}
    assert adapter._model_input_debug_records == []
...
def test_live_turn_monitor_prints_aggregate_after_selected_game_cadence() -> None:
    ...
    monitor.emit(_completed_turn(game_index=1, duration=2.0, completed_levels=2))
    ...
    assert "avg_model_sec_agent=4.000" in rendered
    assert "avg_model_sec_change=0.000" in rendered
