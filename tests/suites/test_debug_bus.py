...
def test_live_turn_monitor_prints_aggregate_after_selected_game_cadence() -> None:
    ...
    monitor.emit(ModelCallCompleted(role="historizer", duration_seconds=8.0))
    monitor.emit(ModelCallCompleted(role="world", duration_seconds=10.0))
    monitor.emit(ModelCallCompleted(role="agent_probing", duration_seconds=12.0))
    monitor.emit(ModelCallCompleted(role="agent_policy", duration_seconds=14.0))
    ...
    assert "avg_turn_sec_hist=8.000" in rendered
    assert "avg_turn_sec_world=10.000" in rendered
    assert "avg_turn_sec_probing=12.000" in rendered
    assert "avg_turn_sec_policy=14.000" in rendered
