...
def test_openai_responses_client_builds_final_provider_request() -> None:
    ...
    result = adapter.create_response(
        ...
        tools=[{"type": "function", "name": "submit_action"}],
    ...
    assert request["tools"] == [{"type": "function", "name": "submit_action"}]
...
def test_openai_responses_client_can_skip_max_tool_calls_for_agent_x() -> None:
    client = FakeClient(SimpleNamespace(id="resp-agent", output=[]))
    adapter = OpenAIResponsesClient(
        OpenAIResponsesImageConfig(max_tool_calls=3),
        client=client,
    )
    ...
    assert "max_tool_calls" not in client.responses.calls[0]
