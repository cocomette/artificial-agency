...
def test_known_model_profiles_use_normalized_1000_coordinates() -> None:
    profiles = [
        ...
        resolve_model_vision_profile(backend="ollama", model="gemma4:e2b"),
    ...
    assert {profile.coordinate_space for profile in profiles} == {"normalized_1000"}
    assert {profile.source for profile in profiles} == {"model_profile"}
    assert [profile.input_image_size for profile in profiles] == [
        (1024, 1024),
        (1024, 1024),
        (256, 256),
        (256, 256),
    ...
    assert [profile.bbox_order for profile in profiles] == [
        "xyxy",
        "xyxy",
        "yxyx",
        "yxyx",
    ...
    assert {profile.axis_frame for profile in profiles} == {
        "top_left_x_right_y_down"
    }
