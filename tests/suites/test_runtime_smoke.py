...
class FakeAgent:
    ...
    def decide(
        ...
        del tool_runtime, world_game_context, goal_game_context, recent_action_history
...
class CapturingAgent(FakeAgent):
    ...
    def __init__(self) -> None:
        ...
        self.world_game_contexts: list[str] = []
        self.goal_game_contexts: list[str] = []
    ...
    def decide(
        ...
        self.world_game_contexts.append(world_game_context)
        self.goal_game_contexts.append(goal_game_context)
...
def _runtime_models(
    *,
    ...
    return ModelRegistry(
        ...
        goal_prediction_model=goal_prediction_model or FakeGoalPredictionModel(),
...
def test_environment_shell_model_inputs_debug_trace_prints_sanitized_inputs(
    ...
    orchestrator = Orchestrator(
        ...
        models=_runtime_models(
            ...
            goal_prediction_model=InspectableGoalPredictionModel(),
    ...
    assert "goal provider input" in trace
    assert "GOAL PROVIDER PROMPT" in trace
...
def test_game_loop_injects_updater_agent_context_on_next_x_call(tmp_path) -> None:
    ...
    assert agent.world_game_contexts == ["world-2"]
    assert agent.goal_game_contexts == ["goal-2"]
...
def test_game_loop_persists_updater_contexts_from_post_decision_predictions(
    ...
    assert len(goal_model.contexts) == 3
    assert experiments == []
...
def test_game_loop_uses_distinct_updater_task_slots(tmp_path) -> None:
    ...
    assert goal_updater.goal_calls == 3
...
def test_real_step_turn_metrics_reaches_updater_and_persists_to_m(
    ...
    assert metrics.score_delta == 1.0
    states = state.list_states(game_id="game-1")
    assert states[0].turn_metrics.time_cost == 1.0
    assert states[0].turn_metrics.score_delta == 1.0
...
def test_animation_frame_predictions_and_turn_metrics_reach_updaters(
    ...
    assert updater.goal_inputs[1].synthetic_none_action is not None
    assert updater.goal_inputs[1].synthetic_none_action.is_none()
    assert updater.world_inputs[0].actual_next_observation is not None
    assert updater.goal_inputs[1].actual_next_observation is not None
    assert (
        updater.world_inputs[0].post_decision_predictions.world_prediction
        is not None
    )
    assert (
        updater.goal_inputs[1].post_decision_predictions.goal_prediction
        is not None
    )
    ...
    assert [
        input_.current_turn_goal_game_context for input_ in updater.agent_inputs
    ] == ["", "goal-1", "goal-2"]
    assert [
...
def test_real_post_decision_predictions_call_models_and_persist_to_m(
    tmp_path,
) -> None:
    database = SQLiteDatabase(tmp_path / "runtime.sqlite")
    state = StateMemory(database)
    ...
    assert [observation.id for observation in goal_model.observations] == ["obs-reset"]
    ...
    assert states[0].goal_prediction["predicted_description"] == {
        "goal_from": "obs-reset"
    }
...
def test_non_controllable_frames_run_post_decision_predictions(tmp_path) -> None:
    ...
    assert [observation.id for observation in goal_model.observations] == [
        "obs-reset-frame-0",
        "obs-reset-frame-1",
        "obs-reset-frame-2",
    ]
...
def test_game_loop_runs_world_goal_models_after_agent_decision(tmp_path) -> None:
    database = SQLiteDatabase(tmp_path / "runtime.sqlite")
    state = StateMemory(database)
    experimental = ExperimentalMemory(database)
    world_model = FakeWorldPredictionModel()
...
def test_shell_model_registry_wires_openai_roles_explicitly() -> None:
    ...
    assert isinstance(registry.goal_prediction_model, GoalPredictionAdapter)
    assert registry.updater_tasks.world_game_updater.provider.backend == "openai"
    assert registry.updater_tasks.goal_game_updater.provider.backend == "openai"
...
def test_shell_model_registry_wires_updater_task_configs_independently() -> None:
    ...
    assert registry.updater_tasks.goal_game_updater.config.instruction_dir == "g"
...
def test_shell_model_registry_wires_real_world_goal_updaters() -> None:
    registry = shell._build_model_registry(
        agent_config=_openai_agent_config(),
            world_config=_openai_tool_config(),
            goal_config=_openai_tool_config(),
    ...
    assert registry.updater_tasks.world_game_updater.provider.backend == "openai"
    assert registry.updater_tasks.goal_game_updater.provider.backend == "ollama"
    assert registry.updater_tasks.agent_game_updater.provider.backend == "openai"
    assert registry.updater_tasks.general_updater.provider.backend == "openai"
...
def test_shell_model_registry_applies_shared_vlm_to_local_roles() -> None:
    ...
    assert isinstance(registry.goal_prediction_model, GoalPredictionAdapter)
    assert registry.goal_prediction_model.config.model == "qwen3.6"
    assert registry.goal_prediction_model.config.keep_alive == "30m"
    assert registry.goal_prediction_model.config.options == {
        "temperature": 0,
        "num_ctx": 8192,
        "num_predict": 500,
    }
