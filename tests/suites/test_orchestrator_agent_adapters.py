...
def test_openai_agent_prompt_places_agent_context_in_user_text() -> None:
    ...
    assert "- `ACTION2`: down." in instructions
...
def test_openai_agent_prompt_includes_recent_action_history() -> None:
    ...
    assert (
        ...
        "1. ACTION6 {\"x\": 500, \"y\": 313} [changed_pixel_percent=3] "
        "change: The coordinate action had no useful effect.\n"
        "2. ACTION2 [latest] [changed_pixel_percent=7] change: The cursor moved down."
    ) in prompt
    assert "Action outcome evidence" not in prompt
    assert "stagnation_warning" not in prompt
    assert "## Attached frames" not in prompt


def test_openai_agent_prompt_renders_recent_action_history_relative_to_crop() -> None:
    first, current = _observations()
    ...
    prompt = _input_text(client.responses.calls[0])
    assert '1. ACTION6 {"x": 0, "y": 1000} [latest]' in prompt


def test_openai_agent_prompt_includes_action_suppression_evidence() -> None:
    first, current = _observations()
    client = FakeOpenAIClient([_openai_final_response("resp-final")])
    adapter = OpenAIOrchestratorAgentAdapter(client=client)
    ...
    adapter.decide(
        ...
        first_observation_ref=ObservationRef(memory="state", id=first.id),
        action_outcome_evidence=ActionOutcomeEvidence(
            suppression_threshold=3,
            suppressed_actions=("ACTION6",),
            suppression_reason=(
                "ACTION6 was omitted because recent uses were "
                "low-information."
            ),
    ...
    assert (
        "## Action suppression evidence\n\n"
        "- suppression_threshold: 3\n"
        "- suppressed_action_choices: ACTION6\n"
        "- suppression_reason: ACTION6 was omitted because recent uses were "
        "low-information."
    ) in prompt
...
def test_agent_prompt_includes_game_reset_history_marker() -> None:
    ...
    prompt = _input_text(client.responses.calls[0])
    assert (
        "1. ACTION1 [changed_pixel_percent=2] change: The agent moved.\n"
        "2. GAME_RESET [reason=game_over_reset] [restart_count=1]\n"
        "3. ACTION2 [latest] [changed_pixel_percent=5] "
        "change: The new game frame changed."
    ) in prompt


def test_agent_prompt_includes_score_advance_history_marker() -> None:
    first, current = _observations()
    action_entry = ActionHistoryEntry(
        action=ActionSpec(action_id="ACTION1"),
        controllable=True,
        changed_pixel_percent=9,
        change_summary="The level ended.",
    )
    score_marker = ActionHistoryScoreAdvanceMarker(
        previous_score=None,
        new_score=1.0,
        delta=None,
    )
    ...
    adapter.decide(
        ...
        [ActionSpec(action_id="ACTION1")],
        FakeRuntime(available_tools=()),
        recent_action_history=(action_entry, score_marker),
        first_observation_ref=ObservationRef(memory="state", id=first.id),
    )

    prompt = _input_text(client.responses.calls[0])
    assert (
        "1. ACTION1 [changed_pixel_percent=9] change: The level ended.\n"
        "2. SCORE_ADVANCE [latest] [previous_score=null] [new_score=1.0] "
        "[delta=null]"
    ) in prompt
...
def test_agent_prompt_marks_animation_history_entries() -> None:
    ...
    prompt = _input_text(client.responses.calls[0])
    assert (
        "1. ACTION1 [changed_pixel_percent=4] change: The cursor moved right.\n"
        "   animation_after:\n"
        "   - NONE [animation] [latest] [changed_pixel_percent=0] "
...
def test_ollama_agent_prompt_includes_recent_action_history_changed_pixel_percent() -> None:
    first, current = _observations()
    history_entry = ActionHistoryEntry(
        action=ActionSpec(action_id="ACTION1"),
        controllable=True,
    ...
    assert (
        "1. ACTION1 [latest] [changed_pixel_percent=5] change: A tile flashed."
    ) in prompt
...
def test_openai_agent_applies_arc_grid_crop_after_resize() -> None:
    first, _ = _observations()
    frame = Image.new("RGB", (4, 4), color=(10, 20, 30))
    for x in range(4):
        frame.putpixel((x, 3), (200, 210, 220))
    current = Observation(id="obs-current", step=2, frame=frame)
    client = FakeOpenAIClient([_openai_final_response("resp-final")])
    adapter = OpenAIOrchestratorAgentAdapter(
        OpenAIOrchestratorAgentConfig(
    ...
    image_url = _input_images(client.responses.calls[0])[0]["image_url"]
    image = _decode_data_url_image(image_url)
    assert image.size == (4, 3)
    assert all(
        image.getpixel((x, y)) == (10, 20, 30)
        for x in range(4)
        for y in range(3)
    )
...
def test_parse_action_scales_crop_relative_normalized_coordinates() -> None:
    crop_edges = 4

    edge_action = parse_action(
        {
            "action_id": "ACTION6",
            "data": {"x": 0, "y": 1000},
        },
    ...
    assert edge_action.data == {"x": 4, "y": 60}
    assert midpoint_action.data == {"x": 32, "y": 32}
