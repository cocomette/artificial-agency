...
def test_openai_agent_ignores_provider_reasoning_summaries() -> None:
    ...
    assert first_prompt["frames"] == [
        {"label": "history_anchor"},
        {"label": "current"},
    ]
    assert first_prompt["recent_action_history"] == []
...
def test_openai_agent_uses_world_goal_contexts_when_tool_budget_is_zero() -> None:
    first, current = _observations()
    ...
    adapter.decide(
        ...
        world_game_context="world L",
        goal_game_context="goal L",
    )

    request = client.responses.calls[0]
    prompt = json.loads(request["input"][0]["content"][0]["text"])
    assert prompt["world_game_context"] == "world L"
    assert prompt["goal_game_context"] == "goal L"
...
def test_openai_agent_prompt_includes_recent_action_history() -> None:
    ...
    adapter.decide(
        ...
        world_game_context="world-game-only",
        goal_game_context="goal-game-only",
    ...
    assert "`frames`" in system_instructions
    assert "`requires_data: true`" in system_instructions
    assert "`requires_data: false`" in system_instructions
    assert "Action glossary:" in system_instructions
    assert (
        "`ACTION6`: coordinate action targeting `x,y` on the 64x64 game grid."
        in system_instructions
    )
    assert "Follow the active `allowed_actions` schema" in system_instructions
    assert "`ACTION7`: undo-style simple action." in system_instructions
    assert "authoritative list of actions" in normalized_system_instructions
    assert "AGENT_CONTEXT:" in system_instructions
    assert "agent K" in system_instructions
    assert "agent L" in system_instructions
    assert prompt["world_game_context"] == "world-game-only"
    assert prompt["goal_game_context"] == "goal-game-only"
    assert prompt["allowed_actions"] == [
        {"action_id": "ACTION1", "data": None, "requires_data": False}
    ]
    assert prompt["recent_action_history"] == [
        {
            "action": {
                "action_id": "ACTION1",
                "data": None,
    ...
    assert prompt["frames"] == [
        {"label": "history_anchor"},
        {"label": "current"},
    ]


def test_agent_prompt_keeps_recent_action_history_compact() -> None:
    first, current = _observations()
    history_entry = ActionHistoryEntry(
        action=ActionSpec(action_id="ACTION1"),
        controllable=True,
...
def test_ollama_agent_uses_structured_final_action_format() -> None:
    ...
    assert "Output contract: return exactly one JSON value" in client.calls[0][
        "messages"
    ][0]["content"]
