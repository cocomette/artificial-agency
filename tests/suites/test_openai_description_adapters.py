...
def test_openai_world_prediction_composes_prompt_and_returns_result() -> None:
    ...
    assert "Your task is to understand the current environment mechanics" in request[
        "instructions"
    ]
...
def test_openai_goal_prediction_composes_prompt_and_returns_result() -> None:
    ...
    assert prompt is not None
    assert adapter.last_instructions is not None
    assert "Your task is to understand the current goal hypothesis" in request[
        "instructions"
    ]
    assert "ROLE_CONTEXT:" in prompt
