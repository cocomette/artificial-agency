...
The active runtime model calls are change summary, world model, historizer,
and updater P for agent game context. Agent X adapters remain in the tree, but
Agent X is dormant in the current game loop; updater P selects the actions that
orchestration queues and submits to the environment.
...
- `vllm/vllm_h100_qwen36_35b_fp8_debug.yaml`: Modal H100 vLLM debug loop
  copied from the Kaggle RTX 6000 debug settings with Modal-local paths.
- `vllm/vllm_modal_rtx6000_qwen36_35b_fp8_debug.yaml`: Modal RTX PRO 6000
  vLLM debug loop based on the H100 Qwen debug settings.
...
`agent_action_history_window` is retained for the dormant Agent X path. The
active world and historizer roles use separate action-history windows:
`world_action_history_window` and `historizer_history_window`. They default to
`20` prior controllable action groups, including bundled synthetic `NONE`
animation evidence, real environment actions, and changed-pixel percentages
from the change-summary visible frame area. Set either to `0` to disable prior
action history for that model input.

`updater_actions_window` controls exactly how many ordered actions the agent
updater must return in `next_actions`. It defaults to `1`.
...
historizer, and the active agent updater. Agent X is dormant; `models.agent`
may remain in old configs but is not used by the current game loop. For a local
Ollama run, configure the active updater role:
...
Each Modal launch resolves the local git commit id and writes run artifacts
under `/vol/runs/<commit-id>/` in the `face-of-agi-runs` Volume. The default
SQLite path is `/vol/runs/<commit-id>/memory.sqlite`; parallel runs derive
per-game files in the same folder. While a run is active, the Modal runner
commits the run volume every 30 seconds by default so debug tools can inspect
the latest committed SQLite files. Change the interval, or disable live commits
with `0`, through `--live-commit-seconds`. The remote runtime shell's stdout
and stderr are streamed through Modal logs while the run is active.

The sample Modal H100 config uses vLLM for transition change summaries, world,
historizer, and the active updater task. Modal starts `vllm serve` inside the
H100 container and the runtime talks to `http://127.0.0.1:8000/v1` through Chat
...
Modal runs default to `H100`. A config can override the accelerator with
`modal.gpu`, for example `RTX-PRO-6000` in
`vllm/vllm_modal_rtx6000_qwen36_35b_fp8_debug.yaml`.
...
as `--max-num-seqs 1` when you want batched parallel inference. Failed games are
recorded once while the batch continues processing other selected games. Runtime
entrypoints may pass `RuntimeConfig.deadline_monotonic`, which is not a YAML
setting, to stop cleanly before a platform hard timeout.
...
dashboard in Modal mode. The Modal run folder / commit id defaults to the
current commit. `Pull snapshot now` downloads every SQLite file in that Modal
folder matching `^memory-game-index-.*\.sqlite$` into the local memory database
folder shown in the sidebar. Live Play and Offline Inspector then use the local
memory file selector from that folder, like normal offline inspection:

```bash
uv run --group debug --with modal streamlit run debug/dashboard/app.py -- --modal
```

Use `--modal-volume`, `--modal-run-folder`, `--modal-database`,
`--modal-snapshot`, or `--local-database` after Streamlit's `--` separator for
non-default Modal Volume, run folder / commit id, SQLite file name, local
snapshot path, or local-run database path.
