...
active roles use separate action-history windows:
`world_action_history_window`, `historizer_action_history_window`,
`probing_action_history_window`, and `policy_action_history_window`. Each
defaults to `8` prior controllable action groups, including bundled synthetic
`NONE` animation evidence, real environment actions, and changed-pixel
percentages from the change-summary visible frame area. Set any of them to `0`
to disable prior action history for that model input.
