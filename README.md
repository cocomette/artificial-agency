...
Reward Judge. World, Interest, and Agent X can receive shared online LoRA
updates through the vLLM runtime.
...
- `uv sync --extra ml --no-dev`: runtime plus vLLM, Torch, TRL, PEFT, and
  offline LoRA training dependencies.
...
reward weights, or LoRA update budget.
...
The download command preloads the non-FP8 `Qwen/Qwen3.6-35B-A3B` trainer base
into the Modal model volume. The FP8 served model remains inference-only for
online LoRA.

The Modal app mounts two Volumes:

- `face-of-agi-local-models` at `/vol/models` for vLLM/Hugging Face model
  caches, adapters, and Diffusers caches.
...
World, Goal, and Reward Judge. Modal starts `vllm serve` inside the H100
container with LoRA enabled and the runtime talks to
`http://127.0.0.1:8000/v1` through Chat Completions. If a config defines
...
disabled thinking in the chat template, and runtime LoRA updating.
...
Goal estimates, Memory documents, LoRA update attempts, and raw redacted JSON
for inspection.
