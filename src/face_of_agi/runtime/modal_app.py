...
MODAL_VLLM_PACKAGE_ENV = "FACE_OF_AGI_MODAL_VLLM_PACKAGE"
MODAL_TRANSFORMERS_PACKAGE_ENV = "FACE_OF_AGI_MODAL_TRANSFORMERS_PACKAGE"
MODAL_EXTRA_PIP_PACKAGES_ENV = "FACE_OF_AGI_MODAL_EXTRA_PIP_PACKAGES"
DEFAULT_MODAL_VLLM_PACKAGE = "vllm==0.23.0"
DEFAULT_MODAL_TRANSFORMERS_PACKAGE = "transformers>=5.7.0,<6"
...
def ollama_models_from_config_text(config_text: str) -> tuple[str, ...]:
    ...
    for role_name in ("agent", "change", "historizer", "memory"):
...
def _modal_pip_packages() -> tuple[str, ...]:
    """Return image-time pip packages for optional model-specific runtimes."""

    vllm_package = (
        os.environ.get(MODAL_VLLM_PACKAGE_ENV, DEFAULT_MODAL_VLLM_PACKAGE).strip()
        or DEFAULT_MODAL_VLLM_PACKAGE
    )
    transformers_package = (
        os.environ.get(
            MODAL_TRANSFORMERS_PACKAGE_ENV,
            DEFAULT_MODAL_TRANSFORMERS_PACKAGE,
        ).strip()
        or DEFAULT_MODAL_TRANSFORMERS_PACKAGE
    )
    extra_packages = tuple(
        package.strip()
        for package in os.environ.get(MODAL_EXTRA_PIP_PACKAGES_ENV, "").split(",")
        if package.strip()
    )
    return (vllm_package, transformers_package, *extra_packages)
...
if modal is not None:
    ...
    class ModalGameRunner:
        ...
        def _start_vllm(self, config: VLLMServerConfig) -> None:
            ...
            _patch_minicpm_v46_vllm_processor(config)
...
def _patch_minicpm_v46_vllm_processor(config: VLLMServerConfig) -> None:
    """Patch vLLM 0.23.0 MiniCPM-V 4.6 processor compatibility if needed."""

    model_text = f"{config.model} {config.model_path or ''}".lower()
    if "minicpm-v-4.6" not in model_text and "minicpm_v_4.6" not in model_text:
        return
    ...
    subprocess.run(
        [
            sys.executable,
            "-c",
            _MINICPM_V46_VLLM_PROCESSOR_PATCH,
        ],
        check=True,
        env={**os.environ, **_modal_env()},
    )
...
_MINICPM_V46_VLLM_PROCESSOR_PATCH = r"""
from __future__ import annotations

from pathlib import Path
import importlib.util

spec = importlib.util.find_spec("vllm.model_executor.models.minicpmv4_6")
if spec is None or spec.origin is None:
    raise RuntimeError("unable to find vLLM minicpmv4_6 module to patch")

path = Path(spec.origin)
text = path.read_text(encoding="utf-8")
marker = "MiniCPM-V 4.6 keeps the native transformers MiniCPMV4_6Processor"
if marker in text:
    print("vLLM MiniCPM-V 4.6 processor patch already present")
    raise SystemExit(0)

target = "    def get_model_version(self):\n"
if target not in text:
    raise RuntimeError(f"unable to locate MiniCPMV4_6ProcessingInfo patch point in {path}")

method = '''    def get_hf_processor(self, **kwargs: object):
        ...
        # so it is incompatible with the legacy vendored MiniCPMVProcessor.
...
'''
path.write_text(text.replace(target, method + target), encoding="utf-8")
print(f"patched vLLM MiniCPM-V 4.6 processor compatibility in {path}")
"""
