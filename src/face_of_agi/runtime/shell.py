...
def _build_orchestrator(
    ...
    *,
    ...
    compacter_config: ModelRoleConfig | None = None,
    ...
    return Orchestrator(
        ...
        or _build_model_registry(
            ...
            compacter_config=compacter_config or ModelRoleConfig(),
...
def _build_model_registry(
    *,
    ...
    compacter_config: ModelRoleConfig,
    shared_vlm_config: ModelRoleConfig | None = None,
    updater_config: UpdaterRuntimeConfig | None = None,
) -> ModelRegistry:
    ...
    compacter_role_config = _with_shared_vlm_role_config(
        compacter_config,
        shared_vlm_config,
    )
    return ModelRegistry(
        ...
        compacter=_build_compacter_model(
            compacter_role_config,
            ollama_client=shared_ollama_client,
        ),
...
def _build_compacter_model(
    config: ModelRoleConfig,
    *,
    ollama_client: object | None = None,
) -> object | None:
    """Build the selected agent compacter adapter."""

    if config.backend is None or config.backend == "":
        raise ValueError("models.compacter.backend is required")
    backend = config.backend.lower()
    if backend == "openai":
        _require_model("compacter", backend, config)
        return OpenAICompacterAdapter(
            OpenAICompacterConfig(
                **_config_kwargs(config, OpenAICompacterConfig)
            )
        )
    if backend == "ollama":
        _require_model("compacter", backend, config)
        return OllamaCompacterAdapter(
            OllamaCompacterConfig(
                **_config_kwargs(config, OllamaCompacterConfig)
            ),
            client=ollama_client,
        )
    if backend == "vllm":
        _require_role_model("models.compacter", backend, config)
        return VLLMCompacterAdapter(
            VLLMCompacterConfig(
                **_config_kwargs(config, VLLMCompacterConfig)
            )
        )
    if backend in {"huggingface", "huggingface-diffusers", "diffusers"}:
        raise NotImplementedError(
            "Hugging Face compacter provider is not implemented yet"
        )
    if backend == "configurable":
        raise NotImplementedError(
            "Configurable compacter provider is not implemented yet"
        )
    raise ValueError(f"unknown compacter backend: {config.backend}")
...
def _build_updater_tasks(
    ...
    *,
    ...
    return UpdaterTaskRegistry(
        agent_updater=_build_updater_task(
            "agent",
            config.agent,
...
def _require_prompt_updater_task(task_name: str, backend: str) -> None:
    ...
    if task_name != "agent":
        raise NotImplementedError(
            f"{backend} updater is implemented only for the agent prompt task"
        )
