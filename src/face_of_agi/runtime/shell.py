...
def _build_model_registry(
    *,
    ...
    world_role_config = _with_shared_ollama_role_config(world_config, shared_vlm_config)
    goal_role_config = _with_shared_ollama_role_config(goal_config, shared_vlm_config)
    return ModelRegistry(
        orchestrator_agent=_build_agent(
            _with_shared_ollama_role_config(agent_config, shared_vlm_config),
        ...
        goal_prediction_model=_build_goal_prediction_model(
            goal_role_config,
            ollama_client=shared_ollama_client,
        ),
        updater_tasks=_build_updater_tasks(
            _with_shared_ollama_updater_config(updater_config, shared_vlm_config),
            ollama_client=shared_ollama_client,
        ),
    )
...
def _build_updater_tasks(
    ...
    *,
    ...
    return UpdaterTaskRegistry(
        ...
        goal_game_updater=_build_updater_task(
            "goal",
            config.goal,
            ollama_client=ollama_client,
        ),
...
def _with_shared_ollama_role_config(
    config: ModelRoleConfig,
    shared: ModelRoleConfig,
) -> ModelRoleConfig:
    """Apply shared local VLM defaults to an Ollama role config."""

    if (config.backend or "").lower() != "ollama":
        return config
    if (shared.backend or "").lower() != "ollama":
        return config
...
def _with_shared_ollama_updater_config(
    config: UpdaterRuntimeConfig | None,
    shared: ModelRoleConfig,
) -> UpdaterRuntimeConfig | None:
    """Apply shared local VLM defaults to Ollama updater task configs."""

    if config is None:
        return None
    return UpdaterRuntimeConfig(
        world=_with_shared_ollama_role_config(config.world, shared),
        goal=_with_shared_ollama_role_config(config.goal, shared),
        agent=_with_shared_ollama_role_config(config.agent, shared),
        general=_with_shared_ollama_role_config(config.general, shared),
    )
