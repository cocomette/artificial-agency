...
def run_updaters(
    ...
    *,
    ...
    bounded_prior_action_history = bounded_action_history(
        session.action_history,
        window=environment_config.action_history_window,
    ...
    with runtime_timing.span(
        ...
        next_actions, updater_mode = apply_context_updates(
            ...
            action_history_window=environment_config.action_history_window,
...
def enqueue_agent_creator_request(
    session: GameLoopSession,
    *,
    state_memory: StateMemory | None,
    agent_creator_service: AgentCreatorService | None,
) -> None:
    """Queue creator review after the completed M row is durable."""

    if agent_creator_service is None or state_memory is None:
        return
    if not session.state_record_ids:
        return
    agent_creator_service.enqueue_game(
        run_id=session.config.run_id,
        game_id=session.game_id,
        memory_database_path=state_memory.database.path,
    )
