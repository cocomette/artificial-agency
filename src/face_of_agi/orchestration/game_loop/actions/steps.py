...
def run_post_decision_predictions(
    ...
    *,
    ...
    """Run S/G predictions for the current session turn."""
...
def resolve_next_snapshot(session: GameLoopSession, *, debug: DebugBus) -> None:
    ...
    prior_levels_completed = (
        session.current_info.levels_completed
        if session.current_info is not None
        else None
    )
    ...
    session.next = FrameTurnSnapshot(
        ...
        recent_action_history=tuple((*session.action_history, history_entry)),
    )
...
def require_predictions(session: GameLoopSession) -> PostDecisionPredictions:
    if session.predictions is None:
        raise RuntimeError("game-loop session is missing S/G predictions")
...
def _run_post_decision_predictions(
    *,
    ...
    """Run S/G predictions after X chooses a frame action."""
    ...
    debug.capture_model_inputs(frame_context, turn_id, runner.goal_model)
    return predictions
