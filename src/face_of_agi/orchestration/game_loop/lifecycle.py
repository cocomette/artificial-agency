"""Run lifecycle actions for the online-learner game loop."""
...
from face_of_agi.contracts import ActionHistoryResetMarker, GameRunResult, RuntimeConfig
...
def stop_session(
    ...
    *,
    ...
    """Set the terminal result consumed by the run exit path."""
...
def finish_run(session: GameLoopSession, *, debug: DebugBus) -> GameRunResult:
    """Emit stop trace and return the terminal run result."""

    if session.terminal_result is None:
        raise RuntimeError("game-loop session finished without a terminal result")
    debug.emit(RunStopped(session.terminal_result))
