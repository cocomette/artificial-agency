...
def evaluate_observed_transition(
    ...
    *,
    ...
    update_input.action_history_entry = replace(
        update_input.action_history_entry,
        reward=reward,
        reward_judge_notes=judge.notes,
        reward_error_tags=judge.error_tags,
    )
...
def compute_immediate_turn_reward(
    *,
    ...
    return TurnReward(
        ...
        learning_progress=prediction_accuracy,
        ...
        metadata={
            ...
            "learning_progress_proxy": "reward_judge_prediction_accuracy",
            **resource_metadata,
        },
    )
...
def _memory_ledger_entries(
    ...
    *,
    ...
    """Return the Memory-facing ledger with compact reward feedback."""
    ...
    return tuple(
        MemoryLedgerEntry(
            ...
            reward_feedback=_memory_reward_feedback(entry),
        )
        for entry in session.turn_ledger
    )


def _memory_reward_feedback(entry: TurnLedgerEntry) -> dict[str, Any] | None:
    reward = entry.reward
    if reward is None:
        return None
    judge = entry.judge_scores[0] if entry.judge_scores else None
    feedback: dict[str, Any] = {
        "total": reward.total,
        "learning_progress": reward.learning_progress,
    ...
    if judge is not None:
        if judge.notes:
            feedback["judge_notes"] = _compact_notes(judge.notes)
        if judge.error_tags:
            feedback["error_tags"] = tuple(judge.error_tags)
    return feedback


def _compact_notes(value: str, *, limit: int = 180) -> str:
