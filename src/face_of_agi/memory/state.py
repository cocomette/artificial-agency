...
class StateMemory:
    ...
    def complete_frame_turn_state(
        ...
        *,
        ...
        agent_creator_action_history: tuple[ActionHistoryItem, ...] | None = None,
        ...
        if agent_creator_action_history is not None:
            metadata["agent_creator_action_history"] = [
                _action_history_metadata(item) for item in agent_creator_action_history
            ]
    ...
    def read_latest_complete_state_for_run(
        self,
        *,
        game_id: str,
        run_id: str,
    ) -> MStateRecord | None:
        """Read the newest complete M state scoped to one run and game."""

        return self.database.read_latest_complete_m_state_for_run(
            game_id=game_id,
            run_id=run_id,
        )
