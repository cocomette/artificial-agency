...
class PromptUpdaterAdapter:
    ...
    def update_world_game_context(
        ...
        return self._update_context(
            ...
            images=_transition_observation_images(
                update_input.previous_observation,
                update_input.actual_next_observation,
                role="world game",
    ...
    def update_agent_game_context(
        ...
        payload = _prompt_payload(
            ...
            task="agent_game",
            previous_context=update_input.previous_context,
            input_payload=_agent_game_input_payload(update_input),
        )
        payload["attached_images"] = [
            {
                "label": image.label,
                "source": f"transition.{source}",
            }
    ...
    def _update_context(
        ...
        *,
        ...
        request = PromptUpdateRequest(
            target=target,
            instructions=load_updater_instructions(
                task=task,
                role=role,
        ...
        validated = validate_with_repair(
            ...
            validate=parse_updated_context_output,
...
def parse_updated_context_output(text: str) -> str:
    ...
    try:
        ...
        raise UpdaterOutputError(
            "updater response must be JSON with an 'updated_context' string; "
            f"raw response preview: {preview!r}"
        ) from exc
    if not isinstance(loaded, dict):
        raise UpdaterOutputError(
            "updater response must be a JSON object with an 'updated_context' string"
        )
...
def _agent_game_input_payload(
    update_input: AgentGameContextUpdateInput,
) -> dict[str, Any]:
    """Return the exact transition payload visible to the agent updater."""

    return _prompt_jsonable(
        {
            "previous_observation": update_input.previous_observation,
            "current_observation": update_input.current_observation,
            "current_turn_world_game_context": (
                update_input.current_turn_world_game_context
            ),
            "current_turn_goal_game_context": (
                update_input.current_turn_goal_game_context
            ),
            "previous_turn_world_game_context": (
                update_input.previous_turn_world_game_context
            ),
            "trace": update_input.trace,
...
def _transition_observation_images(
    previous_observation: Observation | None,
    current_observation: Observation | None,
    *,
    role: str,
    frame_scale: int,
) -> tuple[PromptImage, PromptImage]:
    if previous_observation is None:
        raise ValueError(f"{role} updater requires a previous observation")
