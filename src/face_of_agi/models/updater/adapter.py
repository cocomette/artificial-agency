    ...
    *,
    ...
    sections.extend(
        [
            ...
            "## Same past state detected\n\n"
            + _same_past_state_detections_text(
                update_input.same_past_state_detections
            ),
            "## Action history\n\n"
...
def _same_past_state_detections_text(
    detections: tuple[SamePastStateDetection, ...],
) -> str:
    if not detections:
        return "none"
    lines = [
        (
            "If this is not empty, it means you were in this exact state before, "
            "meaning all you did was simply running in circle. find out the "
            "strategy evolution that lead you to this state, the strategy you "
            "planned."
        )
    ]
    for index, detection in enumerate(detections, start=1):
        lines.extend(
            [
                f"{index}.",
                "   probing_strategy: "
                + _inline_or_none(detection.probing_strategy),
                "   policy_strategy: " + _inline_or_none(detection.policy_strategy),
                "   probing_evolution: "
                + _inline_or_none(detection.probing_evolution),
                "   policy_evolution: "
                + _inline_or_none(detection.policy_evolution),
            ]
        )
    return "\n".join(lines)
...
def _inline_or_none(value: str | None) -> str:
    return _text_or_none(value).replace("\n", " ")
