...
### Output guidance
In `policy_strategy`:
- You describe in `policy_strategy` the goals to reach. You describe the goal based on the observed elements / areas on the `current_observation_frame` and the `World model`, `Probing evolution`, `Policy evolution`, and `Strategy summary` understanding.
