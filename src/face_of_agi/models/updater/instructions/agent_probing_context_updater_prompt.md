...
- `Strategy summary`: very compressed summary of tried strategies, successes, failures, and changed strategic assumptions.
...
- You are also responsible to bring the gaming-agent out of a stuck/oscillating situation when necessary. Check `Action history`, `World model`, `Probing evolution`, `Policy evolution`, and `Strategy summary` to spot stuck behaviors.
- Help yourself from the `World model`, `Probing evolution`, `Policy evolution`, and `Strategy summary` to get a compact understanding of how things work and the previous strategies.
