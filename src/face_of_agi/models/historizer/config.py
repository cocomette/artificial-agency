"""Configuration for the historizer role."""
...
from face_of_agi.models.historizer.contracts import historizer_summary_json_schema


def openai_historizer_text_format(
    schema: dict[str, Any] | None = None,
    *,
    name: str = "historizer_summary",
) -> dict[str, Any]:
    ...
    return {
        "format": {
            ...
            "name": name,
            "strict": True,
            "schema": schema or historizer_summary_json_schema(),
        }
    }


def with_openai_historizer_text_format(
    text: dict[str, Any] | None,
    *,
    schema: dict[str, Any] | None = None,
    name: str = "historizer_summary",
) -> dict[str, Any]:
    """Force the OpenAI historizer response schema while preserving options."""

    return {
        **(text or {}),
        **openai_historizer_text_format(schema, name=name),
    }


@dataclass(slots=True)
class HistorizerConfig:
    """Provider-neutral config for the historizer."""
...
class OpenAIHistorizerConfig(HistorizerConfig):
    ...
    text: dict[str, Any] = field(default_factory=openai_historizer_text_format)
...
class OllamaHistorizerConfig(HistorizerConfig):
    ...
    format: str | dict[str, Any] | None = field(
        default_factory=historizer_summary_json_schema
    )
