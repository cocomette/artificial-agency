...
def component_instruction_text(base_instruction: str) -> str:
    """Append the ARC rendered-color legend to component instructions."""

    return base_instruction.strip() + "\n\n" + arc_color_legend_text()


def arc_color_legend_text() -> str:
    """Return ARC symbol values mapped through the active ARC renderer."""

    lines = ["ARC rendered color legend:"]
    for symbol, color in arc_rendered_color_map().items():
        lines.append(f"- symbol {_symbol_text(symbol)}: {_rgb_text(color)}")
    return "\n".join(lines)
