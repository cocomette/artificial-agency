...
class VLLMChatClient:
    ...
    def build_request(
        ...
        *,
        ...
        set_optional(request, "timeout", getattr(self.config, "timeout", None))
        merged_extra_body = {
    ...
    def _send_request(self, request: dict[str, Any]) -> Any:
        request_timeout_seconds = _request_timeout_seconds(
            request,
            config_timeout=getattr(self.config, "timeout", None),
        )
        scheduler = getattr(self.config, "scheduler", None)
        context = current_model_call_context()
        provider = str(getattr(self.config, "backend", None) or "vllm")
        model = str(request["model"]) if request.get("model") is not None else None

        def call() -> Any:
            return self._require_client().chat.completions.create(**request)

        if isinstance(scheduler, ModelCallScheduler) and context is not None:
        ...
        return run_unscheduled_provider_call(
            call,
            context=context,
            provider=provider,
            model=model,
...
def _request_timeout_seconds(
    ...
    *,
    ...
    timeout = request.get("timeout", config_timeout)
    if timeout is None:
        return None
    try:
