...
class HFRewardJudgeAdapter:
    """Reward Judge backed by the shared HF/Transformers VLM."""

    def __init__(
        self,
        config: HFRewardJudgeConfig,
        *,
    ...
    def judge_prediction(self, judge_input: RewardJudgeInput) -> RewardJudgeScore:
        ...
        text = self.provider.complete_json(
            ...
            schema_name="reward_judge_score",
            images=(
                hf_observation_image(self.config, judge_input.previous_observation),
                hf_observation_image(self.config, judge_input.current_observation),
            ),
        )
        payload = hf_parse_json_object(text, label="reward judge")
        raw_tags = payload.get("error_tags")
        if not isinstance(raw_tags, list) or any(
            not isinstance(item, str) for item in raw_tags
        ):
