...
def load_environment_config(path: str | Path) -> EnvironmentConfig:
    ...
    if "online_lora" in raw_data:
        raise ValueError(
            "online_lora has been removed; this branch supports static vLLM "
            "inference with reward and proxy learning-progress feedback only"
        )
