...
Active runtime model calls are change summary, world model, agent-context
historizer, and updater P for agent context. Agent X adapters remain in the
codebase, but Agent X is dormant in the current game loop; updater P returns the
actions that orchestration queues and submits.
