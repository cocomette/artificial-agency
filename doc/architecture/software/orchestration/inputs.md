...
Orchestration feeds observed transition evidence into updater `P`, which
revises the agent context for later Agent `X` turns. Agent `X` receives only
its composed agent context, observations, action history, and action space.
World receives resolved observations and actions through its model-role
boundary. Goal model contracts remain present but dormant in normal runtime.
