...
- Controllable final frames produce world description predictions before the
  environment is stepped.
- Non-controllable animation frames produce world description predictions for
  updater input.
...
- Updater receives `last_buffer_frame -> first_new_environment_frame` after a
  real step.
- Updater receives world predictions separately from `AgentTrace`.
- Updater-returned contexts are injected into later `X` and `S` model calls.
