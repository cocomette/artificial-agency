...
- runs compacter and updater selection when the current frame is
...
updater selection runs.

## Updater Action Chains

The agent updater must return exactly `updater_actions_window` actions. During
the middle of a queued action chain, the loop
summarizes the observed transition, appends action-history entries, skips
compacter/updater calls, and submits the next queued action.

When the queue is exhausted, the next observed transition runs the compacter
and updater sequence before choosing another action. If a queued action is no longer present in the current action space,
the queue is abandoned and the full context refresh runs immediately for the
current frame.

Level completion is detected in action history by a `completed_levels`
increase. The level-completing transition runs through the compacter with the
current-level action and strategy history, then orchestration stores the
compacter strategy summary as the previous-level strategy context and clears
any remaining queued actions. The next updater call receives that latest
solved-level strategy summary plus the new-level reset notice. `GAME_OVER`
clears queued actions, models the failed
transition, refreshes context, persists the transition, then resets the
environment.
