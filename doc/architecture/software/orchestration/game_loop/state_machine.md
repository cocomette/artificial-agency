...
summary, Memory, World, Goal, Reward Judge, optional online LoRA coordinator,
and debug bus.
...
9. Regenerate Memory from sanitized action/change ledger rows, then call Goal
   again for next-turn state.
10. Persist the completed state row and v1 artifact tables.
11. Poll or schedule online LoRA updates when every trainable role has enough
    new non-held-out replay samples.
12. Advance session cursors.

The internal ledger row is finalized before Memory regeneration, but the Memory
role receives only sanitized `turn_id`, prompt-facing `action`, and
`change_summary` rows. The reward uses prediction accuracy as a bootstrap
proxy; delayed World learning progress is attached later to replay rows after
an online update measures old/new World deltas.
