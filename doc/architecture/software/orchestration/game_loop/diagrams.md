...
flowchart TD
    ...
    Synthetic --> PredictBuffered["RUN_SG_PREDICTIONS<br/>S/G with NONE"]

    PredictBuffered --> SkipEnv[Skip environment step]
    SkipEnv --> UpdateBuffered["RUN_UPDATER<br/>actual = next buffered frame"]

    ValidateReal --> Predict["RUN_SG_PREDICTIONS<br/>S/G from current frame"]
...
sequenceDiagram
    ...
    loop each FrameTurn in buffer
        ...
        alt non-final frame
            ...
            Orch->>Orch: run S/G predictions
            Orch->>Orch: skip environment step
            Orch->>P: compare current frame with next buffered frame
        else final frame
            Orch->>X: decide(agent context, S/G contexts, frame, action_space)
