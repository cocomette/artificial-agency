...
Memory ledger. Each Memory ledger row contains `turn_id`, prompt-facing
`action`, `change_summary`, and concise reward feedback when available:
`total`, `learning_progress`, `prediction_accuracy`, `goal_delta`,
`progress_bonus`, `resource_cost`, and short judge notes/error tags. Game-over
reset does not replace the first frame or clear prior ledger entries;
orchestration adds an explicit reset marker whose action/change summary are
preserved in the sanitized rows. Candidate tables, goals, and ledger metadata
remain internal/persisted orchestration artifacts and are not sent to Memory.
