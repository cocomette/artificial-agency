...
Reward Judge scores, Goal predictions, separated reward components, replay
samples, and online LoRA update attempts. These tables are part of `M` and are
reset with the run database. Within one run, game-over reset does not clear the
