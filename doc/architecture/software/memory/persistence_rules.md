...
- Candidate predictions, judge scores, rewards, replay samples, and LoRA
  attempt state are written to their explicit v1 tables. Reward metadata stores
  resource-cost components, and `TurnMetrics` stores aggregate model prompt,
  completion, and total token counts for the turn.
