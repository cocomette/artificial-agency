...
5. When fresh context is needed, update world-model context from transition
   ...
   mode, apply the deterministic probing cap, read same-run prior rows with
   the current frame hash, and run the selected updater P agent game task for
...
7. Persist the completed turn, decision trace, transition metrics, and agent
   context into `M`.
8. At run end, run updater P for agent general context.
