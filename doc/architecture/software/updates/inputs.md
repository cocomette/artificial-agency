...
- historizer strategy evolution containing `probing_evolution` and
  `policy_evolution`
- same past state detections, when prior complete rows in the same run have the
  same `current_frame_hash` after the change-summary ARC-edge crop; each item
  contains `probing_strategy`, `policy_strategy`, and stored historizer
  evolution fields, but no prior action
