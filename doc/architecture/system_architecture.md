...
- Memory regenerates a free-form run document from first/current frames and a
  sanitized action/change ledger, preserving reset history.
- World predicts candidate transition summaries.
- Goal estimates the current goal, subgoals, and remaining steps.
- Interest scores candidate actions for expected LP and task progress.
...
artifacts, replay samples, and LoRA update attempts. `E` stores generic Agent X
tool outputs. Older SQLite run databases are incompatible with the current
schema and should be reset.

## Providers

The v1 active roles are vLLM-only. Runtime configs under
`src/face_of_agi/runtime/configs/` define the active role set and vLLM LoRA
startup options. Online LoRA uses shared per-role versioned adapters across
registered games; World uses image-aware SFT while Interest and Agent use GRPO.
