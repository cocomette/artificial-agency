...
- Agent creator optionally maintains learned role revisions in a separate
  SQLite database.
...
When configured, the agent creator reviews completed game rows asynchronously
and writes learned roles to its own database. Those learned roles are not read
by the historizer or updater path.
...
context. The agent creator store keeps learned roles, creator runs, tool-call
records, and pending game-review requests. `E` stores generic Agent X tool
