...
- `score_delta_{i,t}`: raw environment progress delta when exposed by the
  environment.
...
- Run world and goal predictions for updater evidence.
...
- VLMs for all roles.
- Description-producing model calls for world and goal predictions.
...
```text
X(C^X_{i,t}, L^S_{i,t}, L^G_{i,t}, O_{i,0}, O_{i,t}, action_space_{i,t}) -> A_{i,t}, T^X_{i,t}
```

Where:

- `C^X_{i,t} = K^X + L^X_{i,t}`.
- `L^S_{i,t}` and `L^G_{i,t}` are the maintained world and goal game contexts.
...
```text
P(T^X_{i,t}, D_hat^S, D_hat^G, O_{i,t}, O_{i,t+1}, H_{i,t}, score_delta_{i,t}, L^X_{i,t}) -> L^X_{i,t+1}
```
...
- `A_{i,t}` is the submitted real action or synthetic `NONE` action used by
  the world updater.
- `T^X_{i,t}` is the full agent trace, including the final action and any tool
  calls.
- `H_{i,t}` is transition timing, such as elapsed real steps and Agent X
  decision duration.
- `score_delta_{i,t}` is raw environment progress metadata when available.
...
role-specific prediction, and previous/current observation frame attachments;
the world updater also receives the selected real action or synthetic `NONE`
action. Agent X's prompt updater receives the broader turn metrics: live trace,
S/G descriptions, observed frames, transition timing, and score/progress
metadata. World and goal prompt updaters do not receive timing metadata. The
...
M_i = {
  ...
  score_deltas: score_delta_{i,0}...score_delta_{i,t},
  contexts: {
    K^S, K^G, K^X,
    L^S_{i,0}...L^S_{i,t},
    L^G_{i,0}...L^G_{i,t},
...
used for current S/G predictions.

This memory exists so Agent X tools can store temporary outputs before a real
action is committed. World and goal predictions are stored with the frame turn
in `M_i`.
...
World and goal predictions use the same description schema as
`DESCRIPTION_SCHEMA` in `tests/e2e/core.py`:
...
`action` is required for world predictions and unset for goal predictions.
...
The full trace `T^X_{i,t}` should be passed to the updater, because the updater
for the agent needs to improve how the agent chooses actions. `tool_calls` and
`tool_results` are dormant future-extension fields in the current runtime.
...
`score_delta` is persisted separately as raw environment progress metadata when
the environment exposes it. Orchestration does not compute prediction
discrepancy, goal-distance metrics, or a scoring value for updater input; the
updater models compare
predicted descriptions, actual observations, traces, timing, and score/progress
metadata directly.
...
for each environment step t:
  ...
  assemble turn metrics:
    ...
    score_delta_{i,t} = environment progress delta when available

  store transition in M_i:
    O_{i,t}, A_{i,t}, O_{i,t+1}, T^X_{i,t}, D_hat^S, D_hat^G, E_{i,t}, H_{i,t}, score_delta_{i,t}

  run updater P:
    update L^S_{i,t} -> L^S_{i,t+1} using D_hat^S, O_{i,t+1}, and A_{i,t}
    update L^G_{i,t} -> L^G_{i,t+1} using D_hat^G and O_{i,t+1}
    update L^X_{i,t} -> L^X_{i,t+1} using trace, predictions,
      observations, and turn metrics
...
- World and goal description predictions.
...
- Improve how it uses world and goal prediction feedback.
...
updater compares predicted descriptions, actual observations, the live trace,
tool results, transition timing, and raw score/progress metadata directly.
...
Orchestration Layer
  ...
  |
  |-- S/G prediction router
  |                     |
  |                     |-- C^S_{i,t} --> World Model S --> predicted_description
  |                     |
  |                     |-- C^G_{i,t} --> Goal Model G  --> goal-relevant predicted_description
  |
  |-- final action --> Environment Adapter
  |
  |-- turn metrics + trace --> Updater P
                              |
...
experimental-memory split. S/G predictions use the current committed frame
...
3. Give the agent `C^X`, current S/G game contexts, `O_{i,0}`, `O_{i,t}`, and
   the current action space.
4. Agent emits one final valid action.
5. Run S/G predictions.
6. Environment advances one step.
7. Store the real transition, trace, and predictions in `M_i`.
9. Assemble turn metrics for Agent X's prompt updater and persistence.
